...
class Config(object):
    ...
    def elasticsearch(self):
        if self.get('_elasticsearch') is None:
            self._elasticsearch = self.setup_elasticsearch()
        return self._elasticsearch
    
    def setup_elasticsearch(self, host='localhost', port=9200):
        from elasticsearch import Elasticsearch
        return Elasticsearch([{'host': host, 'port': port}])

    @property
    def gcloud_storage(self):
        if self._gcloud_storage is None:
            from google.cloud import storage as gcloud_storage
            self._gcloud_storage = gcloud_storage.Client()
        return self._gcloud_storage
    ...
    def load_spark_session(self, appName="sparkApp", mem='80g', showConsoleProgress=False, additional_conf=[], logLevel=None):
        ...
        import pyspark
        if logLevel:
            logging.getLogger('').getChild('py4j').setLevel(logLevel)
        else:
            logging.getLogger('').getChild('py4j').setLevel(logging.WARNING)
