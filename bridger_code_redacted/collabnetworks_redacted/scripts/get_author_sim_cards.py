...
DESCRIPTION = """Save author cards to JSON files, for authors with similar tasks/methods to a seed author"""
...
def save_author_card(outdir, data, name, author_id, s2_id=None, min_year=None, max_year=None, overwrite=True, basedir_name='author_sim'):
...
def main(args):
    if args.outdir is None:
        outdir_base = Path(ROOT_DIR).joinpath('data/visgraph_data/author_sim')
    else:
    ...
    for fp in Path(args.dirpath).glob('*.csv'):
        ...
        outdir = outdir_base.joinpath(f"{name}_{args.min_year}-{args.max_year}_sim_{label}")
        if outdir.exists():
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--outdir", default=None, help="base output directory (default: data/visgraph_data/author_sim")
