...
DESCRIPTION = (
    """Use multiprocessing and subprocessing to run data formatting in parallel"""
)
...
PATTERN_JSONL = re.compile(r"titles_abstracts_dygie_format_(\d+)")

SCRIPT = "./scripts/format_data_for_dygie.py"
...
def main(args):
    ...
    for i, fpath in enumerate(fpaths):
        ...
        log_file = logdirpath.joinpath(
            "format_data_for_dygie_{:06d}_{}.log".format(i, today)
        )
        outfpath = outdir.joinpath(
            "titles_abstracts_dygie_format_{:06d}.jsonl".format(i)
        )
        subprocess_args.append((fpath, outfpath, log_file, args.expand_abbreviations))
